...
import { getTestSuiteConfig, logCurrentConfig } from '../automation/config/rate-limit-config';
...
test.describe('Rate Limited E2E Tests', () => {
  ...
  test('should make API calls with rate limiting and caching', async ({ page }) => {
  ...
  test('should handle search with rate limiting', async ({ page }) => {
    ...
    await PlaywrightRateLimitUtils.smartWait(
      ...
      async () => {
        ...
        const hasResults = await page.locator('[data-testid="city-card"], .city-card').count() > 0;
        return hasResults;
      },
      {
        timeout: 15000,
  ...
  test('should handle authentication flow with rate limiting', async ({ page }) => {
  ...
  test('should handle multiple concurrent requests with rate limiting', async ({ page }) => {
    ...
    const apiCalls = [
      ...
      PlaywrightRateLimitUtils.apiCallWithRateLimit(page, '/api/cities?search=Bangkok', {
        priority: 'medium',
        cacheTtl: config.caching.defaultTtl
      })
    ];
  ...
  test('should handle errors gracefully with rate limiting', async ({ page }) => {
