...
test.describe("Visual Inconsistencies Testing", () => {
  ...
  test("1. Filter accordion transparency and overlapping issues", async ({
    page,
  }) => {
    ...
    const filterButton = page.locator(
      '[data-testid="filter-button"], .filter-button, button:has-text("Filter")'
    );
    const filterAccordion = page.locator(
      '[data-testid="filter-accordion"], .filter-accordion, .accordion'
    );
    ...
    if ((await filterButton.count()) > 0) {
      ...
      const filterContainer = page
        .locator('.filter-container, .filters, [class*="filter"]')
        .first();
  ...
  test("2. Sign-in and sign-up buttons styling inconsistencies", async ({
    page,
  }) => {
    ...
    const signinButton = page.locator(
      'button:has-text("Sign in"), button:has-text("Login"), [type="submit"]'
    );
    ...
    const signupButton = page.locator(
      'button:has-text("Sign up"), button:has-text("Register"), [type="submit"]'
    );
  ...
  test("3. Color inconsistency on sign-in page vs other pages", async ({
    page,
  }) => {
    ...
    // Compare colors
    const colorMismatch =
      homepageColors.backgroundColor !== signinColors.backgroundColor ||
      homepageColors.color !== signinColors.color;

    if (colorMismatch) {
      console.log(
        "⚠️ Color inconsistency detected between homepage and sign-in page"
      );
    }
    ...
    const accessibilityIssues = await page.evaluate(() => {
      ...
      headings.forEach((heading, index) => {
        ...
        if (currentLevel > previousLevel + 1) {
          issues.push(
            `Heading hierarchy skip detected at heading ${index + 1}`
          );
        }
  ...
  test("6. Navigation bar misalignment when user is logged in", async ({
    page,
  }) => {
    ...
    const navbarLoggedOut = await page
      .locator("nav, .navbar, header")
      .first()
      .evaluate(el => {
  ...
  test("7. Missing home page navigation option from settings page", async ({
    page,
  }) => {
