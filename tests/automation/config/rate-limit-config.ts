...
export function getTestSuiteConfig(suiteName: string) {
  ...
  const suiteConfig =
    (TEST_SUITE_CONFIGS as any)[suiteName] || TEST_SUITE_CONFIGS["e2e-testing"];
  ...
  return {
    ...
    rateLimits: {
      ...
      delayBetweenRequests: Math.floor(
        baseConfig.rateLimits.delayBetweenRequests /
          suiteConfig.rateLimitMultiplier
      ),
    },
    mocking: {
      ...baseConfig.mocking,
    ...
    parallelism: {
      ...
      maxWorkers: Math.min(
        baseConfig.parallelism.maxWorkers,
...
export function logCurrentConfig(): void {
  ...
  console.log(
    `   Rate Limits: ${config.rateLimits.maxRequestsPerMinute} req/min, ${config.rateLimits.delayBetweenRequests}ms delay`
  );
