...
class DatabaseSynchronizer {
  ...
  private async createBackup(): Promise<string> {
    ...
    try {
      ...
      console.log(
        `✅ Backup created successfully: ${cities.length} cities, ${users.length} users, ${reviews.length} reviews`
      );
  ...
  private async syncReferenceData(options: SyncOptions): Promise<{
    ...
    try {
      ...
      console.log(
        `📊 Synced: ${result.cities} cities, ${result.users} users, ${result.reviews} reviews`
      );
  ...
  private async getDatabaseStats(prisma: PrismaClient, label: string) {
    try {
      ...
      console.error(
        `❌ Failed to get ${label.toLowerCase()} database stats:`,
        error
      );
  ...
  async synchronize(options: SyncOptions = {}): Promise<SyncResult> {
    ...
    try {
      ...
      const devStatsBefore = await this.getDatabaseStats(
        devPrisma,
        "Development"
      );
      ...
      const devStatsAfter = await this.getDatabaseStats(
        devPrisma,
        "Development"
      );
      ...
      console.log(`\n📈 Changes Summary:`);
      console.log(
        `   Cities: ${devStatsBefore.cities} → ${devStatsAfter.cities} (+${devStatsAfter.cities - devStatsBefore.cities})`
      );
      console.log(
        `   Users: ${devStatsBefore.users} → ${devStatsAfter.users} (+${devStatsAfter.users - devStatsBefore.users})`
      );
      console.log(
        `   Reviews: ${devStatsBefore.reviews} → ${devStatsAfter.reviews} (+${devStatsAfter.reviews - devStatsBefore.reviews})`
      );
