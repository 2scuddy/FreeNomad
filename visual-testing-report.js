...
async function runVisualTests() {
    ...
    for (const selector of filterSelectors) {
      ...
      if (await filterButton.count() > 0) {
        ...
        try {
          ...
          console.log(`⚠️ Could not click filter with selector ${selector}: ${error.message}`);
        }
      }
    }
    ...
    for (const route of authRoutes) {
      try {
        ...
        if (!pageContent.includes('404') && !pageContent.includes('Not Found')) {
          ...
          await page.screenshot({ 
            path: path.join(screenshotDir, `2-auth-page-${route.replace('/', '')}.png`),
            fullPage: true 
          });
          
          // Check for auth buttons
          const authButtons = await page.locator('button, input[type="submit"]').all();
          const buttonStyles = [];
    ...
    for (const route of settingsRoutes) {
      try {
        ...
        if (!pageContent.includes('404') && !pageContent.includes('Not Found')) {
          ...
          await page.screenshot({ 
            path: path.join(screenshotDir, `6-settings-page-${route.replace(/\//g, '-')}.png`),
            fullPage: true 
          });
          
          // Check for home navigation options
          const homeLinks = await page.locator('a[href="/"], a:has-text("Home")').count();
          const breadcrumbs = await page.locator('.breadcrumb, .breadcrumbs, nav[aria-label="breadcrumb"]').count();
          const backButton = await page.locator('button:has-text("Back"), .back-button').count();
  ...
  const report = {
    ...
    summary: {
      ...
      foundIssues: findings.filter(f => f.status === 'Found' || f.status === 'Tested').length,
  ...
  findings.forEach((finding, index) => {
    ...
    if (finding.accessibilityIssues && finding.accessibilityIssues.length > 0) {
      console.log(`   Accessibility Issues: ${finding.accessibilityIssues.join(', ')}`);
    }
  });
